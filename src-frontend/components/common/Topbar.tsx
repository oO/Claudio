...
export const Topbar: React.FC<TopbarProps> = ({
        ...
        <Popover
          ...
          content={
            <div className="space-y-3 max-w-xs">
              ...
              <ActionButton
                icon={Settings}
                label="Select Claude Installation"
                variant="outline"
                size="sm"
                onClick={onSettingsClick}
                className="w-full"
  ...
  return (
    <motion.div
      ...
      className={cn(
        "flex items-center justify-between px-4 py-3 border-b border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 relative",
        className
      )}
    >
      <DebugLabel label="Topbar" />
      {/* Status Indicator */}
      <StatusIndicator />
