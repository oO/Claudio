    ...
    const handleCopySessionId = async () => {
      if (sessionFilePath) {
        try {
          ...
          console.log(`Session absolute path copied to clipboard: ${sessionFilePath}`);
        } catch (error) {
    ...
    const handleCopyCollapsedUuids = async () => {
      if (collapsedMessageUuids && collapsedMessageUuids.length > 0) {
        try {
          ...
          console.log(`Copied ${collapsedMessageUuids.length} collapsed message UUIDs to clipboard`);
        } catch (error) {
