...
# Claudio UserPromptSubmit Hook - Sets status to active and clears any transient events
INPUT=$(cat)
...
if [ -n "$SESSION_ID" ]; then
    ...
    echo "=== UserPromptSubmit Hook $(date -u +"%Y-%m-%dT%H:%M:%SZ") ===" >> "$LOG_FILE"
...
if [ -n "$PROJECT_ID" ]; then
    ...
    if [ -f "$SESSION_FILE" ]; then
        # Set status to "active" and update hook with full payload
